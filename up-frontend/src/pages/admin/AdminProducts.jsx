        ...
        <div className="col" style={{ flex: 1, minWidth: 360 }}>
          <table className="table">
            ...
            <tbody>
              {ordered.map((p) => (
                <tr key={p.id}>
                  <td><strong>{p.name}</strong><div className="small">{p.slug}</div></td>
                  <td>{p.category}</td>
                  <td>{p.price}€</td>
                  <td>
                    <input
                  ...
                  <td>
                    <div className="row wrap">
                      ...
                      <button className="btn" onClick={() => setFeatured(p.id)}>{p.featured ? "Vedette" : "Mettre vedette"}</button>
              ...
              {ordered.length === 0 ? (
                <tr><td colSpan="6" className="muted">Aucun produit.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>

        <div className="col" style={{ width: 420, maxWidth:"100%" }}>
          ...
          <div className="row wrap">
            ...
            <button className="btn primary" onClick={() => {
              ...
              next.images = [String(next.images?.[0] || "").trim() || blank.images[0]];
              upsert(next);
              setEditing(blank);
            }}>Enregistrer</button>
          </div>
          ...
          <div className="row wrap">
            ...
            <div style={{ width: 140 }}>
              <label>Prix (€)</label>
              <input className="input" type="number" min="0" value={editing.price} onChange={(e)=>setEditing({ ...editing, price: Number(e.target.value || 0) })} />
            </div>
          </div>
          ...
          <div>
            ...
            <input className="input" value={editing.images?.[0] || ""} onChange={(e)=>setEditing({ ...editing, images: [e.target.value] })} />
          </div>
          ...
          <div className="row wrap" style={{ alignItems:"center" }}>
            ...
            <label style={{ display:"flex", gap: 8, alignItems:"center" }}>
              <input type="number" min="0" className="input" style={{ width: 120 }} value={editing.stockQty}
                onChange={(e)=>setEditing({ ...editing, stockQty: Number(e.target.value || 0) })} />
              Stock
            </label>
          </div>
        </div>
